...
class tasTest:
    ...
    def setHeadphoneVol(self, vol):
        volBin = "{0:b}".format(vol) + "110"
        setVolStr = "Set Headphone Volume %s %s " % (str(vol), str(volBin))
        self.writeData(0x77, int(volBin, 2), setVolStr)  # Set volume
        self.dumpData()
...
if __name__ == "__main__":
    ...
    tt.startSequence()
    tt.setVolume(0x40)
    tt.setHeadphoneVol(0x0)
    os.system(
        "aplay -Dplughw:sndrpisimplecar /home/mycroft/mycroft-core/mycroft/res/snd/start_listening.wav"
    )
    tt.close()
